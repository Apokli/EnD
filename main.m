...
plot_pd(pd2_1, A2, labels, "actual Pd when unknown A and s", "A");    % plotted against A (not SNR)
...
for i = 1 : length(Pfa)
    for j = 1 : size(A2, 2)
        detector = abs(squeeze(x2(:, :, j)) * s);             % kick A out of detector
        pred2_2(i, :, j) = double(detector > threshs2(i)); % predicts
    end
end

% Calculate Pds
pd2_2 = zeros(size(pd));             % probability of prediction (5 * 30)
for i = 1 : size(pd2_2, 1)
    for j = 1 : size(pd2_2, 2)
        pd2_2(i, j) = sum(pred2_2(i, mask == 1, j)) / sum(mask);     % predicted positives / real positives
    end
end
plot_pd(pd2_2, A2, labels, "actual Pd when unknown A and s", "A");    % plotted against A (not SNR)
